...
def delete_empty_file(full_filepath):
   ...
   if is_empty_file(full_filepath) and name_of_file.strip() != "Icon":
...
def organize_files(sourcedir):
   ...
   for entry in scandir(sourcedir):
      ...
      if entry.is_file():
         ...
         if delete_empty_file(file_path) or ext.strip() == "":
            continue 
         ...
         for dir in dic.keys():
            ...
            if ext in dic[dir]:
               ...
               notf_ext= True # extension not found is true  
               
         if notf_ext: # extension not found 
